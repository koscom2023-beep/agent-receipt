...
■ 리서치 근거검증(출처 대조 — git 아님·검증만·공유 Evidence Kernel):
  research verify --file <report.json|report.md> [--fetch] [--out <p>] [--decompose]   각 주장의 15종 근거(인용·수치·범위·날짜·링크·해시·서명·commit·변경파일·diff·스키마·버전·파일·영수증·산출물)를 결정론 대조 · .md=동결 문법 입력 · --fetch=라이브 재대조 · --out=Verification Receipt(입력해시+provenance 봉인)
...
function main(): void {
  ...
  if (command === "research") {
    ...
    if (process.argv[3] === "verify") {
      void runResearchVerify(getArg("--file"), { fetch: hasFlag("--fetch"), out: getArg("--out"), decompose: hasFlag("--decompose") });
      return;
    }
  ...
  if (command === "gate") {
    // gate: 검사 실패=행위 중단 집행(R9·opt-in). fail→exit1(또는 --hook 시 PreToolUse deny)·gate오류=exit2(non-block).
    runGate(getArg("--receipt"), { hook: hasFlag("--hook"), vendor: getArg("--vendor") });
  }
  if (command === "council") {
