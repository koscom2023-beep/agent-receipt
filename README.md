...
Current version **`0.22.0`** (`@promptia-labs/agent-receipt`). **`0.22.0`** — **UX pass: onboarding, one face, first three seconds** (2026-07-08, a UI/UX council — all surfacing/reorganizing, zero new features, identity untouched): the tool has many commands, so this release makes the common path obvious without hiding the power. **(Onboarding)** the no-args first run now **welcomes** a new user (contract-less) with a one-line mental model and points at `quickstart`, instead of a bare "no contract" line; `quickstart` names the **three core verbs** `begin → done → share` up top and **demotes `capture` to optional/later** (it no longer auto-runs on `--write` — the shared-settings hook shouldn't scare a first session). **(One mental model)** a single sentence now leads `help` and the welcome: *this tool leaves two things — what the AI did (Work Receipt) and what the AI said (Verification Receipt)* — dissolving the "why are there two receipts" confusion. **(One face)** the four HTML outputs (share-proof · inbox · dashboard · Evidence Browser) now share **one style SSOT** (`src/htmlstyle.ts`) — a warm-paper palette, one type set, and a verdict-hierarchy color that matches the landing site's thermal-receipt world; the Evidence Browser's mature token set was absorbed as the shared superset. **(First three seconds)** share-proof opens with a **verdict hero** (icon + color + label, scroll-zero) carrying the contract sentence and change magnitude, for the recipient who opens the page without a CLI. **(Accessibility)** every verdict is a **triple signal — color + icon + text** (a single `verdictVisual` source; never color alone), on a contrast-checked palette, HTML still self-contained (no external resources). **(Honesty, folded not removed)** the "tamper-evident / not a compliance guarantee" caveats move into a **one-click `<details>`** so a green result reads big while the full scope stays a click away — honesty by placement, not by shouting. **(i18n skeleton)** a core-path bilingual layer (`--lang en` / `AGENT_RECEIPT_LANG=en`, default `ko` = byte-identical for existing users) so the onboarding path speaks English for global npm users; full translation is a later train. **`0.21.0`** — **MCP tap (observation proxy, alpha)** (2026-07-08, three design councils + code-vs-spec reconciliation): a new **observation source** — `mcp-tap` sits between the agent and its stdio MCP servers and records every `tools/call` **value-free** (top-level key names · byte size · a canonical digest — never the argument values, never the `env` block), hash-chained like `capture`, vendor-neutral. It surfaces in the receipt as `tapSummary` (metadata, **excluded from `contentHash`** — receipts without tap stay byte-identical). Design honesty is enforced in code: the JCS (RFC 8785) canonicalizer earns the name `jcs/sha256` **only if the frozen official test vectors all pass** (a test demotes it to `sorted/1` otherwise); reads coalesce losslessly (`repeat:N`, closed by input-events only — never a timer — so it stays deterministic) while writes/exec/network/deploy are **never merged**; a crash is confessed by a missing `cleanShutdown` marker; the session boundary is a **cursor snapshot** (a long-lived proxy can't truncate its own log without breaking the chain); URIs are split by scheme (file=path / else scheme+host, userinfo stripped) with a `uriDigest`; HTTP/SSE servers are out of v1 scope and **confessed in `coverage.unwrapped`** rather than silently skipped. `tap install / probe / status / show / verify / uninstall` manage the wiring (preview by default; restore truth = a self `sidecar`, not a key in someone else's config). Verified on 5 real servers (filesystem · memory · everything · sqlite · git — [compat table](https://github.com/koscom2023-beep/agent-receipt/blob/v0.1-verify-check-split/docs/MCP_TAP_COMPAT.md)); a CI-witness workflow re-runs the release commit under a separate identity (a witness, **not** a key-trust proof). db-write classification proven on a live sqlite server (lead-token parse). Guard integration is **warn-only** (no tap-path deny), overhead micro-bench is **not yet measured**, and English CLI output is a later train — all stated, none faked. Full design: [MCP_TAP_SPEC.md](https://github.com/koscom2023-beep/agent-receipt/blob/v0.1-verify-check-split/docs/MCP_TAP_SPEC.md). **`0.20.2`** — **integrity patch** (2026-07-07, from a full self-audit): a `review reject` / `needs-review` sidecar is **no longer counted as an approval** — `hasApproval` was existence-only, so a rejected receipt read as approved in four consumers (`attest` in-toto approval count · `ledger` · `commit-check` · `incident`); status reading is now a single SSOT (`approvalStatusFor`: legacy sidecars without `status` still count as `approved`, a broken sidecar counts as nothing), with a 4-case regression test (rejected / needs-review / legacy / corrupt). Also: the site-parity and replay-equivalence tests joined `npm test` (they were manually-run only), and the historical feature-snapshot doc now carries a "not current" banner. No new features. **`0.20.1`** — **docs & polish patch** (2026-07-07): republishes with the review-ready README/site copy that landed after `0.20.0` shipped, the em-dash-free site copy, a `gen-claim` usage line covering all three modes (`--transcript | --llm-prompt | --from-llm`), and the GitHub Action default `version` pin corrected to the published `0.20.0` (was a stale `0.14.0` — pin = consumer reproducibility, kept at a version guaranteed to exist). No engine change. **`0.20.0`** — **review-ready receipts** (2026-07-07 — three council batches in one version; versions bump once per publish): the release closes the loop the market names as the bottleneck — *where did it come from · what was it meant to do · who is responsible*. **(Ten-second proof)** share-proof opens with an **executive block above the tabs**: the verdict, the contract restated as **plain sentences (KO+EN, fixed slots, zero LLM)**, magnitude straight from receipt fields (a test forbids summary-vs-receipt drift), guard-denied count, and the verdict's `[rule-id]` facts regrouped into **four review buckets in the reviewer's real reading order — Scope / Critical paths (as designated by the contract) / Validation / Agent behavior** (pure regrouping: a test asserts the signal set is unchanged; per-bucket cap 5 with the fold stated, empty buckets say so; no built-in "sensitive" lexicon — the contract is the judge, the tool only matches). An optional **session objective** (`begin --objective`) records *what the session was meant to do* — always rendered with a **"self-reported, unverified" prefix**, below the contract sentence, never laundered as fact. A **session timeline** renders deterministically from capture records (last 500, fold stated — also in print), degrading honestly to git commit times. `done` ends with **ordered next steps ①→②→③ per verdict**. The front door compresses to **four verbs** `begin / done / share / review` (+ print-first `quickstart`, `--client` condensed view that names what it omits and points to the full proof of the **same contentHash**). **(Review, recorded)** `review approve | reject | note` formalizes the local review sidecar — hosted-compatible fields frozen (`status / reviewer / reviewedAt / note`), legacy approvals read as `approved`, first bare note becomes `needs-review`; the status badges the **inbox** (filterable), the share-proof ("Review recorded — a record, not an authority"), and the **PR comment**. **(Operate the pile)** `inbox` gains hosted-compatible grouping keys (`branch / sessionId / vendor[known-only] / captureQuality[full·degraded·none] / reviewStatus / repoLabel / checksTotal / checksFailed`) and **fact-named preset filters** ("FAIL only" · "denied-path touched" · "critical paths + no checks" · "unreviewed" — judgment words are banned by test), shareable via URL hash, zero server. The PR side adds a **one-line `--summary` mode** (stdout only, for CI logs) and a **review badge** in the comment (mechanical sidecar projection). Two **policy preset packs** — `client-delivery` and `sensitive-backend` — ship *suggested* lexicons (auth/billing/infra approval paths, publish/push/network action signals) whose files state in comments that **the adopter is the judge, the tool only matches**. Offline **`verify-proof <bundle-dir>`** re-checks a hand-off bundle in one command (seal recompute · DSSE over PAE · Rekor sidecar shape · evidence replay) with plain-language failure reasons; **action classes** (`git push`/`npm publish`/network by **lead-token parse** — a quoted "git push" stays `other`) feed an opt-in **`forbid_actions`** guard (fail-open; the deny message confesses it is a signal, not a lock). **`0.19.0`** — **share & sign batch** (2026-07-07, P2 + backlog): **share-proof is now a one-page, three-tab proof** — Change / Evidence / Cost (the same three audits the product leads with), tabs in **pure CSS** (zero `<script>` — the security test still bans script tags), **print expands every pane** (tabs never hide evidence on paper), and an empty tab says honestly how to fill it instead of staying silent (`--evidence-dir` embeds a neutral pass/fail rollup of the session's verification receipts; `--with-cost` embeds the generation-time session cost estimate — labeled, and never stored in the sealed receipt). **`share-proof --bundle`** emits a hand-off directory — `index.html`, the untouched `receipt.json`, the DSSE envelope + Rekor sidecar + `public.pem` when present, copied `evidence/` receipts, and a `VERIFY.md` that walks the recipient through third-party verification (and says out loud when no anchor exists). Verification receipts can now be **DSSE-signed offline**: `predicate --sign` wraps the `claim-verification/v1` Statement in a DSSE envelope with the *existing* ed25519 key (no second key, same payloadType as `anchor`, zero network — public-log registration stays a separate decision). And `gen-claim` gains an **LLM decomposition bridge that never calls an LLM**: `--llm-prompt` emits a deterministic decomposition prompt with the claim schema embedded (SSOT = `spec --format json`), `--from-llm` normalizes the response through a schema-derived field allowlist (invented fields dropped *and reported*, claims without a statement dropped, an `UNVERIFIED` label sealed into the file) — judgment stays deterministic: `research verify` grades the result and mechanically kills fabricated quotes. **`0.18.0`** — **verdict clarity & cost diagnosis batch** (2026-07-07, P1 of the session-contract spine): **the verdict's rules are now codified** — a `VERDICT_RULES` registry (12 rules, unique ids) plus [docs/VERDICT.md](https://github.com/koscom2023-beep/agent-receipt/blob/v0.1-verify-check-split/docs/VERDICT.md), kept in lockstep by a test (a rule id missing from the doc fails the suite); every reason line now carries its machine-readable rule id (`[denied-path] …`, parse with `^\[([a-z-]+)\]`), and the escalation boundary is stated in code: **a WARN never auto-escalates — FAIL originates only from contract/check violations**. `INCOMPLETE` reasons split into four codes (`no-baseline` / `stale-branch-mismatch` / `stale-baseline-not-ancestor` / `stale-unknown`), each carrying a fixed **how-to-fix** line (mechanical mapping, not an LLM suggestion). A new opt-in policy key **`fail_on_done: fail | incomplete | warn`** turns the verdict into a CI exit gate with threshold semantics (unset = today's behavior; when it fires, `done` confesses *which* threshold raised the exit — and a broken `policy.yaml` is confessed out loud instead of silently disabling the gate). `cost` gains **"why was it expensive"** — a component decomposition (input / output / cache-read / cache-creation, computed with the *same formula* as the total so the two numbers cannot disagree) plus fixed-rule signals that each disclose their own trigger condition; **no judgment words** (a test bans them — facts only, the verdict never reads cost), unpriced models excluded and counted, never estimated; also in `cost --json` as `diagnosis`. **`0.17.0`** — **session verdict batch** (2026-07-07, P0 of the session-contract spine): **`done` now closes a session with a 4-value verdict** — `PASS / PASS_WITH_WARNINGS / FAIL / INCOMPLETE` — a **gate, not a score** (no numbers, no grade averages; a regex test enforces this), rolled up purely from existing signals (verify ok · denied/out-of-scope · required checks · critical paths · policy hits · waste/loop · red flags · baseline state) with per-item facts (`reasons`) backing every verdict, precedence `FAIL > INCOMPLETE > WARN > PASS`, and `INCOMPLETE` pinned to a fixed definition (no baseline / stale baseline). A **contract snapshot** (kind · allowed-glob count · denied list · advisory forbidden actions · budget + `contractHash` pointer) now leads the `done` header, the receipt markdown, and **share-proof** — the session's contract is the first thing a reader sees. Both fields are **metadata excluded from the receipt `contentHash`** (old receipts stay byte-identical; the integrity model is untouched). Evidence grades gain **plain-language labels** (`A` Strong (recomputed) · `B` Source-matched · `C` Format-only · abstain Not-enough-basis) shown *alongside* the technical grade, never replacing it. Exit codes unchanged. **`0.16.1`** — **docs & site** (2026-07-06): this README, the landing site (KO/EN), and the internal system map updated to document the `0.16.0` verification-depth batch (the `bench` harness, A/B/C grades + `abstain`, the Merkle transparency log, the in-toto predicate, the regulation crosswalk, `otel`, the action-halt `gate`, the model-swap audit, the 15th `range` check). **No code change** — `0.16.0`'s tarball shipped before these docs landed, so this patch republishes with them. **`0.16.0`** — **verification-depth batch** (2026-07-06): a **`bench` harness** that puts a number on the checker itself (precision/recall on a labeled set + **N-run reproducibility** — same input, same verdict; reproducibility break → `exit 1`; seed-set accuracy is *in-sample*, never a world-accuracy claim); **evidence grades A/B/C + `abstain`** on every check (**A** recompute [number/hash/signature] > **B** compare [citation/git/schema] > **C** form; a claim with no basis to judge abstains instead of being over-assured — additive to the kernel, existing `verified`/`failed` byte-invariant); a **Merkle transparency log** (RFC6962 — leaf/node domain separation, O(log n) inclusion proofs, and **consistency proofs that catch a re-chained past** a plain hash-chain can't; `ledger merkle` runs it over the real append-only ledger's `entryHash` leaves — honest label: a transparency *data structure*, not a live log service); an **in-toto `claim-verification/v1` predicate** (`predicate` wraps a Verification Receipt as an in-toto Statement, `subject.digest` = the sealed receipt hash; a type *name* for interop, not a hosted registry); a **regulation crosswalk** (`controls --crosswalk` maps each verification capability to **EU AI Act / SOC 2 / ISO 42001** clauses as *evidence relevant to*, per-clause "does not prove" — never a compliance claim); **claim decomposition** (`research verify --decompose` — a compound claim's assertions each get their own graded sub-verdict); **+10 recompute ops** (median/count/abs/pow/mod/variance/stddev/floor/ceil/round — all params-free, population variance, half-up round); **OpenTelemetry emission** (`otel` — OTLP traces/logs, **deps-0** [no SDK bundled], deterministic trace/span ids from the receipt hash); an **action-halt gate** (`gate` — a failed Verification Receipt verdict blocks via CI `exit 1` or a PreToolUse `deny`; opt-in and explicitly wired, tool-error stays non-block to preserve fail-open); a **model-swap audit** (`cost --audit-swap` — declared vs actual model price delta from the pricing table, flags a **downgrade** = premium promised / cheaper served = suspected overcharge; unpriced models stay `unpriced`, no estimation); and a **15th check kind `range`** (a stated number within `[min, max]`, inclusive). All additive and byte-invariant to prior receipts, all deterministic, all with honest scope labels. **`0.15.3`** — **Cursor→WSL bridge** (2026-07-05): the `install-cursor --wsl-bridge` command now emits a working Windows-side `hooks.json` that bridges Cursor (Windows) to agent-receipt (WSL) — completing the wiring `0.15.2` flagged as incomplete. It invokes the tool by absolute node+cli path with `--cd <home>` (the default `wsl.exe -e` working directory is `/mnt/c/Windows`, unwritable — which silently dropped captures), maps Cursor's real `afterFileEdit` event, validates `--distro` against shell injection, prints by default and only writes to a Windows path with an explicit `--windows-hooks <path>` (the Windows username arrives transport-corrupted, so it is never auto-derived), with a `--utf8` opt-in that wraps `chcp 65001`. Verified end-to-end: a real Cursor edit is captured despite BOM/CR/backslash transport corruption; `inspectHookParse` added for gate instrumentation. The robust transport-corruption recovery is unchanged. **`0.15.2`** — **fix batch** (2026-07-03): the **`graph --format json` / Evidence Browser large-output truncation** is fixed (piped/captured output past the ~64KB OS pipe buffer was silently cut to invalid JSON — `console.log`+`process.exit` didn't flush async stdout; a synchronous full-write now guarantees complete output), **`help` now leads with `cost`** (the CLI front door matches the cost-visibility identity; the honest-scope limit lines are preserved), **capture transport-corruption hardening** (hook payloads arriving with a leading UTF-8 BOM / CRLF / codepage-mangled bytes — observed via `wsl.exe` — no longer degrade parsing: strict-parse first, repair fallback only on failure), and an **alpha Cursor multi-vendor adapter** (`capture --vendor cursor` / `install-cursor` normalize Cursor hooks — **experimental; Windows+WSL install wiring is incomplete, don't rely on it yet**). **`0.15.0`** — **cost & control batch** (councils 2026-07-03): the product now leads with **`cost`** — this session's *real* token cost read from Claude Code's local transcript (Anthropic's own per-message usage × list price; an estimate, not an invoice), a per-model split, a **context-bloat → fresh-session** signal, and a **today's-running-total** across the project's sessions (pure summation — the safest "before the month-end bill" number); plus a **real-time scope guard** (opt-in `policy guard: block` denies forbidden-path writes *before the tool runs*; `strict` profile; the denied attempt is chain-recorded), a **loop-intervention** signal (identical test/build/lint reruns with no file change in between), a **repeat-waste** reporter (rereads / duplicate commands / repeated failures), a **change-budget** warn (`contract.budget`), a **review-compression** pass on `prepare-commit` (fixed-tier "look here first" + red-flag observations: test `.only/.skip` and dependency additions), and an **opt-in PR receipt comment** on the GitHub Action (mechanical projection of the sealed receipt, in-place upsert, fork-safe). Honest scope throughout: no fabricated "savings" number — it shows where the money goes and signals when to stop; it does not cut your bill directly. **`0.14.0`** — appeal & impact batch (all 8 council items): the site gains a **hero live-verifier**, a **paste-two-fields simple mode**, **shareable verdict links** (URL *fragment* only — never sent to a server; >8KB refuses instead of silently truncating), and a **fabrication gallery** wired to playground presets; a repo-local **GitHub Action** (`action.yml`, pinned npx, optional `graph diff` regression gate — marketplace listing deliberately deferred); **`badge --receipt`** emits a README badge that links to the receipt's existing **Rekor** `verifyUrl` (no anchor sidecar → refuses: a badge without a public log entry is decoration); `research verify --fetch` now **seals `fetched.textSha256`** (the exact compared text) so **`replay --receipt --fetch`** (opt-in network; default replay stays offline) can answer *"has the cited page changed since verification?"* — ✓ unchanged / ✗ **SOURCE DRIFT** / unreachable; capture adds **`PermissionDenied`** (`(권한 거부됨)` marker) and closes the whole misrecord trap class: **unknown hook events become explicit degraded gap-markers** instead of being silently recorded as successes. **`0.13.0`** is one consolidated release of everything built since the last published `0.12.1` (internal milestones `0.12.2`–`0.17.0` in the commit history were never published and are collapsed into this single version):

- **Evidence Kernel 6 → 14 check kinds** — `commit` / `fileChanged` / `diffContains` (claims checked against actual git history via an IO surface; kernel stays pure), `schema` (pure JSON Schema subset), `version` (**real single-comparator semver satisfaction** — `^3.23.8` correctly verifies `3.24.0`; prerelease/compound → no-basis, never a false verdict), `file` (a claimed path exists), `receipt` (a *cited Verification Receipt* exists and its seal replays clean — a fabricated receipt reference is a `mismatch`), `artifact` (build/test output matches declared shape constraints: size bounds and/or sha256-on-demand; zero constraints → no-basis). Every kind the spec ever listed as "planned" has shipped.
- **Verification UX** — `research verify` accepts **markdown input** (frozen grammar, regex-only, verdicts pinned identical to JSON by an isomorphism test); council verify prints per-claim check detail; ambiguous fingerprint prefixes print per-candidate context.
- **Evidence Browser** — receipts **fold by `receiptId`** across Dashboard/failures/subjects/receipt list (honest `total` vs `fileCount`, `×N` badges — re-running one fixture no longer looks like N problems); Korean-first labels, larger type, muted off-white theme (twice-iterated on direct feedback, contrast computed not eyeballed); `--base-dir <prev>` marks **new failures** vs a snapshot (fields exist only when base is given); a frozen 3-kind **exception rollup** (`seal_failed` / `ungrounded_decision` / `source_unreachable` — reclassification of recorded facts, no workflow states) with a clickable breakdown card that **jumps to the affected receipts**; history precompute capped at 500 with explicit truncation notice.
- **capture** — now sees **WebFetch/WebSearch/Task/`mcp__*`** (name/host-only; prompts, arguments and search queries are values and are never stored) and the documented non-blocking **`PostToolUseFailure`** hook (`--event fail`): a failed attempt is recorded with an explicit `(실패 시도)` marker, never mistakable for a completed action. Two real bugs fixed along the way: an already-installed hook's **stale matcher was never refreshed** (coverage extensions silently never applied), and the old event parser **misrecorded unknown events as successes** — the matcher is now an anchored regex updated in place, and the parser is explicit.
- Self-dogfood covers the byte-stable new kinds; a docs bug (11 kinds captioned "10") is fixed.

**`0.12.1`** is a docs-only patch: this README did not document the verification engine at all — `research` / `council` / `graph` / `spec`, the whole track that shipped in `0.12.0` — even though the code, tests, and CLI help already had it; this fixes that (see [Verification engine](#verification-engine) and [Verification engine commands](#verification-engine-commands)), and renames the six-object model's `Verified Receipt` stage to plain `Receipt` to avoid confusion with the separate `Verification Receipt` kind. **No code changed.** **`0.12.0`** ships the verification engine's consumption layer: a real **Evidence Graph** (`graph view --format json` → `{nodes, edges}` — Receipt/Claim/Check nodes; `asserts`/`checked_by`/`same_input`/`same_commit`/`reverifies` edges, each with a machine-readable `basis` and a `verified`/`reported` trust tier — `verified` only for facts this tool recomputed itself), failure-first triage (`graph failures`), a regression gate (`graph diff` — exit 1 on new failures; per-subject deltas), claim **fingerprints** (`cfp1:` — text-identity v1) with timelines (`graph history --claim|--input|--subject`), a subject status board (`graph subjects`), a failure-first static Evidence Browser with 1-click claim history, self-dogfooding fixtures (every `npm test` accumulates this repo's own verification receipts), and a formalized SDK surface (Stable/Provisional tiers — see [docs/SDK.md](https://github.com/koscom2023-beep/agent-receipt/blob/v0.1-verify-check-split/docs/SDK.md)). `0.10.0` added `schemaVersion`, provenance, the hash-chain ledger, content hashing, and strict-redact; **`0.11.0`** tightens the surface and adds **`capture`** (a beyond-git action trace, *alpha*), **`anchor`** (a third-party Rekor seal + in-proof verification link), and the read-only **`insights`** / **`risk`** / **`controls`** projections; **`0.11.3`** decouples evidence from the gate — a `post-commit` **`receipt --committed`** records the just-made commit even when the pre-commit gate is bypassed with `--no-verify`, and JSON redaction is now structure-aware (valid JSON, no key false-positives); **`0.11.4`** records the agent session (`CLAUDE_CODE_SESSION_ID`) in receipt provenance so a receipt shows which session produced it (a human-run receipt records `null`); **`0.11.5`** reveals the domain model (an object-first README + design docs), unifies the claim↔git comparison on one path rule (Windows-backslash claims no longer false-mismatch), and freezes the git↔capture reconciliation into the receipt (its unexplained-residual count surfaced on the ledger line). Cloud/SaaS, real Slack/webhook transport, remote approval, and any "compliance guarantee" remain intentionally out of scope. Feature coverage vs the design docs: [coverage](https://github.com/koscom2023-beep/agent-receipt/blob/v0.1-verify-check-split/docs/coverage.md).

Version ladder: `0.7.0` (work receipts) → `0.8.0` (AI work audit protocol) → `0.9.x` (convenience + dogfood fixes) → `0.10.0` (integrity: schemaVersion / provenance / hash-chain ledger) → **`0.11.0` (focused: tightened surface + `capture` — beyond-git action trace (alpha) + `anchor` — third-party Rekor seal & in-proof verification link)** → `0.11.1` (patch: executable `npx` bin + heading clarity) → `0.11.2` (patch: security — strip secret-in-URL passwords / redact `export KEY=` & AWS-secret shapes — plus Windows path handling and anchor hardening) → **`0.11.3` (evidence/gate decoupling: `post-commit` `receipt --committed` survives `--no-verify` + JSON-structure-aware redaction)** → `0.11.4` (provenance: receipt records the agent session `CLAUDE_CODE_SESSION_ID`) → **`0.11.5` (domain-model reveal: object-first README + design docs; ClaimDiff path-normalization SSOT fix; git↔capture reconciliation frozen into the receipt + `reconUnexplained` on the ledger)** → **`0.12.0` (Evidence Graph earned: real nodes/edges with basis+tier · failure-first triage/diff/history/subjects · claim fingerprint v1 · self-dogfood receipts · SDK tiers — release notes: [docs/RELEASE-0.12.0.md](https://github.com/koscom2023-beep/agent-receipt/blob/v0.1-verify-check-split/docs/RELEASE-0.12.0.md))** → `0.12.1` (docs-only: README now documents the verification engine that shipped silently in `0.12.0`; `Verified Receipt` renamed to `Receipt` in the six-object diagram) → **`0.13.0` (consolidated: Evidence Kernel 6→14 check kinds incl. git-facts/semver-range/receipt-replay/artifact-shape · markdown report input · receipt folding + Korean-first Evidence Browser · new-failure badges via --base-dir · frozen exception rollup with jump-to-receipt · capture widened to WebFetch/WebSearch/Task/mcp__* + PostToolUseFailure with stale-matcher & misrecord-parser fixes)** → **`0.14.0` (hero live-verifier · paste mode · fragment-only share links · fabrication gallery · repo-local GitHub Action · Rekor-linked `badge` · `--fetch` seal + `replay --fetch` SOURCE-DRIFT detection · `PermissionDenied` capture + unknown-event degraded markers)** → **`0.15.0` (cost & control: `cost` real per-session token cost from the local transcript + today's-running-total + context-bloat signal · real-time scope guard [`guard: block`, strict profile] · loop-intervention · repeat-waste reporter · change-budget warn · review-compression + red-flag observations · opt-in PR receipt comment · positioning led by cost-visibility for solo coders)** → `0.15.1` (patch: `cost` falls back to the newest session *with usage* when the latest transcript file is an empty/summary session — found during promptia setup; robust to git-hook invocations without `CLAUDE_CODE_SESSION_ID`) → **`0.15.2` (patch: `graph --format json` / Evidence Browser large-output pipe-truncation fix · `help` leads with `cost` · capture transport-corruption hardening [BOM/CRLF/codepage] · alpha Cursor multi-vendor adapter — Windows+WSL wiring incomplete)** → **`0.15.3` (Cursor→WSL bridge: `install-cursor --wsl-bridge` emits a working Windows `hooks.json` · `--cd` writable-CWD fix · `afterFileEdit` mapping · print-by-default with opt-in `--windows-hooks` write · `--distro` injection-validated · verified end-to-end against a real Cursor edit)** → **`0.16.0`** (verification depth: `bench` harness [precision/recall + N-run reproducibility] · A/B/C evidence grades + `abstain` · Merkle transparency log [RFC6962 inclusion/consistency] + `ledger merkle` over the real ledger · in-toto `claim-verification/v1` predicate · `controls --crosswalk` [EU AI Act/SOC 2/ISO 42001, evidence-relevant-to] · `research verify --decompose` · +10 recompute ops · `otel` OTLP emission [deps-0] · action-halt `gate` [CI exit 1 / PreToolUse deny] · `cost --audit-swap` model-swap audit · 15th check kind `range`)** → **`0.16.1`** (docs & site: README/site/map updated to document the 0.16.0 batch — no code change) → **`0.17.0`** (session verdict: `done` 4-value gate [PASS/PASS_WITH_WARNINGS/FAIL/INCOMPLETE — not a score, regex-enforced] · contract snapshot leads done/receipt/share-proof · plain-language evidence-grade labels alongside technical grades · all metadata, receipt `contentHash` byte-invariant)** → **`0.18.0`** (verdict clarity & cost diagnosis: `VERDICT_RULES` registry + docs/VERDICT.md lockstep test · `[rule-id]` machine-readable reasons · WARN-never-auto-escalates codified · INCOMPLETE split into 4 fix-annotated codes · opt-in `fail_on_done` exit gate with confession lines · `cost` component decomposition + trigger-disclosed signals, judgment-word-free)** → **`0.19.0`** (share & sign: share-proof 3-tab one-pager [Change/Evidence/Cost · CSS-only tabs, zero script · print expands all panes] · `--evidence-dir`/`--with-cost` embeds · `share-proof --bundle` hand-off directory + VERIFY.md · `predicate --sign` offline DSSE for verification receipts [existing key · zero network] · `gen-claim --llm-prompt`/`--from-llm` LLM decomposition bridge [zero LLM calls · allowlist-normalized · UNVERIFIED-labeled · judged by `research verify`])** → **`0.20.0`** (review-ready receipts: executive block + **4 review buckets in reading order** [signal-set-preserving regroup · contract is the judge] · plain-sentence contract KO/EN · self-reported session `--objective` · deterministic timeline with honest git fallback · ordered ①②③ next steps · four-verb front door + `share`/`--client`/print-first `quickstart` · **`review approve|reject|note`** sidecar [hosted-frozen fields · legacy=approved] badging inbox/proof/PR · `inbox` grouping keys + **fact-named preset filters** shareable by URL hash · PR `--summary` one-liner + review badge · **policy packs** `client-delivery`/`sensitive-backend` [suggested lexicons — adopter is the judge] · offline `verify-proof` one-click · lead-token action classes + opt-in `forbid_actions` guard)** → `0.20.1` (docs & polish: post-release README/site copy · em-dash-free · gen-claim 3-mode usage · Action pin 0.20.0) → **`0.20.2`** (integrity: reject/needs-review sidecars no longer read as approvals across attest/ledger/commit-check/incident — status-aware `hasApproval` via a single SSOT reader, legacy no-status stays approved · site-parity tests join `npm test`). A feature-maximal `1.0.0` is **deferred** in favor of a focused product — see [the version-decision note](https://github.com/koscom2023-beep/agent-receipt/blob/v0.1-verify-check-split/docs/VERSION-DECISION-2026-06-29.md).
