...
| `linked_test_paths` | string[] | optional | `[]` | **0.9** — allowed 밖이지만 직접 가드 테스트(표시 전용, §15). verify 판정 불변 |
| `expected_linked_tests` | string[] | optional | `[]` | **0.9** — 기대 가드 테스트(lint advisory, §15) |
...
## 11.6 receipt 확장 필드 (magnitude / critical paths / contentHash / actions)
...
- **`contentHash`** `"sha256:<hex>"` — Node 내장 `crypto`(sha256, **새 의존성 없음**)로 만든 무결성 해시. 입력은 결정론적 git 실측: `headHash` + `touched`/`staged`/`untracked`/`outOfScope`/`deniedHits`(정렬) + `magnitude` + critical 의 touched + `checks`. **`timestamp`·`environment`·`schemaVersion`·`actions` 는 입력에서 제외**(시간/머신/메타마다 바뀌므로 — 같은 git 상태면 같은 hash). ed25519 **서명은 백로그**(이 버전 미구현).
- **`actions` / `actionsSummary`** (0.11 alpha — `capture`) — `capture`(beyond-git 행위 추적)가 `.agent-guard/capture.jsonl` 에 누적한 행위. **레코드가 있을 때만** receipt 에 등장한다(없으면 키 자체가 없어 **기존 receipt 와 바이트동일** — capture 안 쓰는 사용자 영향 0). `actions[]` = `{ tool, op, path?, host?, flag }` — **값은 저장하지 않는다**(경로/호스트/행위분류만, redact 경유). `actionsSummary` = `{ total, secretFilesRead, externalCalls, createdThenDeleted, gitVisible }`(`gitVisible` = 행위 경로 ∩ git 변경집합 — "git 이 본 것 ⟷ 행위" 대비). **contentHash 입력에서 제외**(metadata·additive — `environment`/`schemaVersion` 과 동일) → capture 를 써도 기존 무결성 해시 불변. 누적 로그는 `capture reset` 으로 초기화. 한계(alpha): 어댑터=Claude Code 단일.
...
11. **v0.9(통합 편의성/안전성 패치)도 `verify --json` 14키(§12)를 바꾸지 않는다.** 계약 스키마에 **optional 필드 2개만 추가**(`linked_test_paths`/`expected_linked_tests`, 둘 다 `default([])` — 기존 계약 불변). 신규 명령: `begin --kind <recon|implementation|docs|test|measure-first|observe-only|release-check>`(세션 종류를 `session.json`·receipt sidecar 에 기록, 14키 비접촉) · `close-recon`(읽기전용 정찰 1발 종료 — 변경 0일 때만 receipt+audit-pack+reset, 구현/dirty 거부) · `prepare-commit`/`finish`(복붙 커밋 블록 생성 — 커밋/reset 블록 물리 분리, **자동 git 0**, denied/진짜 범위 밖/브랜치/NUL 시 블록 생략) · `note --type <recon|contract-draft|no-code-decision|next-options>`(코드 변경 없는 정찰/판단 **메모/주장 — git 검증 아님** → `.agent-guard/notes/`·`/decisions/`, audit-pack 에 참고 포함) · `release-check --base <ref>`(배포 전 read-only 분석 — ahead/behind·changed·risk 휴리스틱·rollbackBase·failed∩changed·postDeployObserve, **push/deploy/checkout/reset 0**) · `next`(상태 기반 단일 명령 추천). 정책: `policy.mode`(standard|measure_first|observe_only, default standard) — commit-check self-report 체크리스트(표시 전용, 게이트 아님). claim 의 `modeClaims`/`externalActions` 는 **self-report(검증 불가)** 로만 표시(mismatch 집계 안 함, `buildPrompt` 필수 JSON 불변). **핵심 하위호환 원칙: 신규 필드/표시는 "정의/존재할 때만 출력"** → 기존 110 golden byte 불변, 신규 케이스(v09-01~29)만 추가. linked test 분류는 **사람용 표시 전용** — verify `outOfScope` 의미는 불변(linked 도 verify 판정에선 oos 그대로, 몰래 allowed 취급 안 함). **금지선 유지**: 자동 commit/push/deploy/revert/npm publish·DB/env·점수화·실 전송·원격 SaaS·새 의존성 0·package-lock 불변. core 추출(`evaluateCommitCheck`/`buildAuditPack`/`resetCore`)은 동작보존(기존 commit-check/audit-pack/reset 출력 불변). `isToolOutput` 제외 집합에 `.agent-guard/notes/`·`/decisions/` 추가. **정체성 격리(0.9 정리 패치)**: git 작업트리만 "증거(evidence)"다. `note`/`decisions`(사용자 메모·주장), `release-check`(배포 판단 보조 — deploy/push/approve 안 함), `policy mode`(self-report 체크리스트 — 강제 아님), claim 의 `modeClaims`/`externalActions`(git 검증 불가 self-report)는 전부 **advisory/주장**이며 **PASS/FAIL 근거가 아니다**. 문서/출력에서 이들에 '검증됨·증명·컴플라이언스 보장·배포 승인' 류 표현을 쓰지 말 것.

10. **v0.8(Sprint 6: AI 작업 감사 프로토콜)도 계약 스키마(§2–§8)와 `verify --json` 14키(§12)를 바꾸지 않는다.** 신규 명령(`begin`/`done`/`policy`/`commit-check`/`trailer`/`audit-pack`/`ledger`/`replay`(별칭 `verify-pack`)/`attest`/`incident`/`report --type`/`export --format github-pr|otel|langfuse`)과 강화(`explain`, `receipt --redact`)는 모두 **verify 판정 스키마와 별개**다: `policy.yaml` 은 **별도 SSOT(`src/policy.ts`)** 이며 계약 스키마에 병합되지 않는다(contract=작업범위, policy=상시규칙). N8 트립와이어(policy 관찰)는 `verify`(사람)/`explain`/`receipt`/`commit-check` 표면에만 싣고 **`verify --json` 14키는 불변**(policy.yaml 이 있어도 — golden s6-06 가드). receipt 에 `environment`/`policy`/`disclosure` 필드가 추가됐으나 **receipt 스키마 한정**이고 `contentHash` 입력에서는 제외(timestamp/environment 제외 → 결정론 유지). `audit-pack`/`ledger.jsonl` 은 sidecar 산출물이라 **`isToolOutput` 제외 집합**에 `.agent-guard/audit-packs/`·`.agent-guard/ledger.jsonl` 을 추가했다(verify 노이즈 방지). `attest` 는 in-toto style 초안(SLSA level 단정 없음), `export` 신규 포맷은 stdout payload preview(전송 없음). **금지선 유지**: 자동 revert/commit·점수화·AI self-cert·실 전송·원격 SaaS·새 의존성 없음. 모든 감사/증거 출력에 한계 고지(git 작업트리 기준; .gitignore·레포밖·OS·DB·외부서비스 못 봄; "컴플라이언스 보장" 아님)를 박았다. golden: 기존 verify(사람)/receipt/explain/report/export 갱신 + s6-01~22 추가(총 110 케이스). 현재 상태 판정은 `docs/coverage.md`.
