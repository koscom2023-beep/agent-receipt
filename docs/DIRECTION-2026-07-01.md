...
Maturity (honest): **L1** individual checker → **L2** shared Evidence Kernel → **L3** versioned Evidence Specification → **L4** registry / plugin → **L5** Verification Receipt + Provenance ← **now** → **L6** Evidence Graph → **L7** SDK / ecosystem → **L8** industry adoption. Graph (L6) is a **query layer built later once receipts accumulate** — not built first. "Evidence VM" and "industry standard" stay future labels.
