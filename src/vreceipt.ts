...
// --fetch(옵트인 네트워크): 영수증에 봉인된 fetched.textSha256(검증 당시 그 페이지의 대조 텍스트 해시)를
// 지금 다시 가져와 재계산 대조 → 출처 표류(source drift) 판정. 기본 replay 는 네트워크 0 그대로.
async function refetchTextSha(url: string): Promise<string | null> {
  const g = globalThis as { fetch?: (u: string, o?: unknown) => Promise<{ ok: boolean; text: () => Promise<string> }> };
  if (typeof g.fetch !== "function") return null;
  try {
    const res = await g.fetch(url, { redirect: "follow" });
    if (!res.ok) return null;
    const { stripHtml } = await import("./research.js"); // 검증 때와 동일 가공(동일 기준 대조)
    return createHash("sha256").update(stripHtml(await res.text())).digest("hex");
  } catch {
    return null;
  }
}

export async function runReplayReceipt(pathArg: string | undefined, opts: { fetch?: boolean } = {}): Promise<never> {
