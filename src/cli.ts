...
자주 쓰는 흐름(이 6개면 충분):
  ...
  agent-receipt next                          지금 할 한 명령 추천
  agent-receipt audit-pack                     증거 묶음 생성(감사 검토용)
  agent-receipt prepare-commit                 커밋 블록 생성(자동 commit/add 안 함)
  agent-receipt explain                        왜 PASS/FAIL 인지 + 이 도구가 못 보는 것

  정찰: begin --kind recon → close-recon      구현: begin --kind implementation → finish → (사람 커밋) → reset
